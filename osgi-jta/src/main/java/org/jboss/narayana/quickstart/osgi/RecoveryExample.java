 ...
 * changed for Fuse by jkasztur@redhat.com
 */
...
public class RecoveryExample implements Action {
	...
	public Object execute() throws Exception {
		if (force) {
			...
			if (tm == null) {
				...
				final DummyXAResource xa = new DummyXAResource(DummyXAResource.FaultType.HALT);
				final Context ctx = new InitialContext();
				final DataSource ds = (DataSource) ctx.lookup("osgi:service/test");
			...
			final File file = new File("DummyXAResource/");
			if (file.exists() && file.isDirectory() && file.listFiles().length > 0) {
				final int currentCommits = DummyXAResource.getCommitRequests();
				...
				// DummyXAResource XA_COMMIT [< formatId=131077, gtrid_length=29, bqual_length=36, tx_uid=0:ffff7f000001:ac37:57070db7:10, node_name=1,
				// branch_uid=0:ffff7f000001:ac37:57070db7:11, subordinatenodename=null, eis_name=0 >] with fault NONE
				//
	...
	private void queryH2Database(String sql) throws Exception {
		...
		try {
			...
			final int rowCount = rs.last() ? rs.getRow() : 0;
